...
public class M2MLocalService extends HttpServlet implements IApplicationService, ISubscriptionManager, IM2MLocalService {
	...
	public static boolean isAnUnconfirmedCommand(AHContainerAddress containerAddress) {
		...
		return containerName.equals(AHContainers.attrId_ah_zigbee_network_status) || ServiceClusterProxy.isAnUnconfirmedCommand(containerAddress);
	}
		...
		public void run() {
			synchronized (m2mLocalListenerList) {
				for (Iterator iterator = m2mLocalListenerList.iterator(); iterator.hasNext();) {
					...
					if (containerAddressFilter == null || HapServiceManager.checkItemsOnContainerIdFilter(cisItems, containerAddressFilter.getM2MContainerAdress()))
						...
						try {
							...
							if (containerAddressFilter == null
								...
								if (!RestClient.isOkOrCreatedStatus(response)) {
									LOG.warn("Removing subscriber: error " + response.getStatusLine().getStatusCode() + " while contacting "
											+ subscriptionInfo.subscription.getContact());
									iterator.remove();
								}
							}
						} catch (Exception e) {
	...
	public void setM2MHapService(IM2MHapService hapService) {
		...
		try {
			...
			serviceClusterProxies = new IServiceCluster[] { onOffClusterProxy, meteringClusterProxy, temperatureMeasurementClusterProxy, iasClusterProxy,
					relativeHumidityMeasurementClusterProxy, illuminanceMeasurementClusterProxy, applianceControlClusterProxy, applianceEventsAndAlertsClusterProxy };
		} catch (Exception e) {
	...
	private ContentInstance postContentInstance(AHContainerAddress containerAddress, ContentInstance ci) throws ApplianceException, ServiceClusterException {
		...
		if (containerName.equals(AHContainers.attrId_ah_zigbee_network_status)) {
			...
			if (containerName.equals(AHContainers.attrId_ah_cluster_ah_ConfigServer_Name) || containerName.equals(AHContainers.attrId_ah_core_config_name)) {
			...
			} else if (containerName.equals(AHContainers.attrId_ah_cluster_ah_ConfigServer_CategoryPid) || containerName.equals(AHContainers.attrId_ah_core_config_category)) {
			...
			} else if (containerName.equals(AHContainers.attrId_ah_cluster_ah_ConfigServer_LocationPid) || containerName.equals(AHContainers.attrId_ah_core_config_location)) {
				...
				if (requestUri.startsWith(subscriptionItemsAddressedId)) {
					...
					if (containerName != null && containerName.equals(AHContainers.attrId_ah_zigbee_network_status)) {
						...
						if (clusterAndAttributeNames == null) {
							...
							IServiceCluster serviceCluster = applianceProxy
									.getServiceCluster(new Integer(containerAddress.getEndPointId()).intValue(), clusterAndAttributeNames[0]);
			...
			try {
				if (requestUri.startsWith(subscriptionItemsAddressedId)) {
					...
					if (!isValidLocalHagId(containerAddress.getHagId()))
						servletResponse.sendError(HttpServletResponse.SC_NOT_FOUND);
					String appliancePid = containerAddress.getAppliancePid();
					if (containerAddress.getAppliancePid() != null && containerAddress.getEndPointId() == null && containerAddress.getContainerName() == null) {
	...
	private void endPointConfigurationUpdated(ApplianceProxy applianceProxy, IEndPoint endPoint, long timestamp) {
		try {
			...
			// hapService.storeAttributeValue(appliancePid, new
			// Integer(endPoint.getId()), ConfigServer.ATTR_NAME_NAME,
			// timestamp, configServer.getName(confirmationRequestContext),
			// true);
			hapService.storeAttributeValue(appliancePid, new Integer(endPoint.getId()), AHContainers.attrId_ah_core_config_name, timestamp,
					configServer.getName(applianceProxy.getApplicationRequestContext()), true);
			// hapService.storeAttributeValue(appliancePid, new
			// Integer(endPoint.getId()), ConfigServer.ATTR_NAME_LOCATION_PID,
			// timestamp, locationPid != null ? new Integer(locationPid) : null,
			// true);
			hapService.storeAttributeValue(appliancePid, new Integer(endPoint.getId()), AHContainers.attrId_ah_core_config_location, timestamp, locationPid != null ? new Integer(
					locationPid) : null, true);
			// hapService.storeAttributeValue(appliancePid, new
			// Integer(endPoint.getId()), ConfigServer.ATTR_NAME_CATEGORY_PID,
			// timestamp, categoryPid != null ? new Integer(categoryPid) : null,
			// true);
			hapService.storeAttributeValue(appliancePid, new Integer(endPoint.getId()), AHContainers.attrId_ah_core_config_category, timestamp, categoryPid != null ? new Integer(
					categoryPid) : null, true);
	...
	public ContentInstance createContentInstance(AHContainerAddress containerAddress, long instanceId, Object content) throws M2MHapException {
		ContentInstance ci = new ContentInstance();
	...
	public ContentInstance createContentInstance(AHContainerAddress containerAddress, ContentInstance contentInstance) throws M2MHapException {
		try {
	...
	public ContentInstancesBatchResponse sendContentInstanceBatchRequest(ContentInstancesBatchRequest contentInstancesBatchRequest) throws M2MHapException {
		try {
