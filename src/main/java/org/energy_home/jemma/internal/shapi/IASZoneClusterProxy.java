...
public class IASZoneClusterProxy extends ServiceClusterProxy implements IASZoneClient {
	public IASZoneClusterProxy(ApplianceProxyList applianceProxyList, AHM2MHapService ahm2mHapService, ISubscriptionManager subscriptionManager) throws ApplianceException {
	...
	public void execZoneStatusChangeNotification(int zoneStatus, short extendedStatus, IEndPointRequestContext context) throws ApplianceException, ServiceClusterException {
		...
		LOG.debug("execZoneStatusChangeNotification: " + zoneStatus + " from appliance " + appliancePid + " (" + peerAppliance.getDescriptor().getType() + "), end point id "
				+ endPointId + " (" + peerEndPoint.getType() + ")");
		Object objectValue = decodeAttributeValue(appliancePid, endPointId, IASZoneServer.ATTR_ZoneStatus_NAME, zoneStatus);
		try {
			sendAttributeValue(appliancePid, endPointId, AHContainers.attrId_ah_cluster_iascontact_open, System.currentTimeMillis(), objectValue, true);
		} catch (Exception e) {
	...
	public ZoneEnrollResponse execZoneEnrollRequest(int ZoneType, int ManufacturerCode, IEndPointRequestContext context) throws ApplianceException, ServiceClusterException {
		...
		LOG.debug("execZoneEnrollRequest: zoneType=" + ZoneType + ", manufacturerCode=" + ManufacturerCode + " from appliance " + appliancePid + " ("
				+ peerAppliance.getDescriptor().getType() + "), end point id " + endPointId + " (" + peerEndPoint.getType() + ")");
