...
public class HapServiceConfiguration {
	...
	public static synchronized void init() {
		...
		if (!Utils.isNullOrEmpty(cachedAttributeIdFilterStr)) {
			// TODO:!!! this is a hack for compatibility with old system
			// property configured in Energy@home trial
		...
		if (Utils.isNullOrEmpty(localOnlyAttributeIdFilterStr))
			// TODO:!!! this is a hack for compatibility with default
			// configuration in Energy@home trial
			// An empty filter ("[]") can be used to send all managed resources
			// to the HAP platform
			// To enable the sending of new AHContainers with Energy@home
			// application a filter must be
			// defined as follows:
			// "[ah.cluster.metering.,ah.cluster.applctrl.,ah.cluster.powerprofile.]"
			// If all delivered and received instantaneous power values have to
			// be sent to the HAP platform, the
			// filter can be defined as follows:
			// "[ah.cluster.metering.deliveredEnergySum,
